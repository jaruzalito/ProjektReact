...
function parseInstagramDescription(description, requestedUsername, pageTitle) {
  ...
  const fullRegex =
    /(\d+(?:[.,]\d+)?[KMBTQ]?|[\d,]+)\s+Followers?,\s*(\d+(?:[.,]\d+)?[KMBTQ]?|[\d,]+)\s+Following,\s*(\d+(?:[.,]\d+)?[KMBTQ]?|[\d,]+)\s+Posts?\s*-\s*(.*?)\s*\(@([\w.]+)\)\s*on\s*Instagram:\s*"([\s\S]*?)"$/i;
  ...
  const basicRegex =
    /(\d+(?:[.,]\d+)?[KMBTQ]?|[\d,]+)\s+Followers?,\s*(\d+(?:[.,]\d+)?[KMBTQ]?|[\d,]+)\s+Following,\s*(\d+(?:[.,]\d+)?[KMBTQ]?|[\d,]+)\s+Posts?/i;
  ...
  const followersMatch = description.match(
    /(\d+(?:[.,]\d+)?[KMBTQ]?|[\d,]+)\s+Followers?/i,
  );
  const followingMatch = description.match(
    /(\d+(?:[.,]\d+)?[KMBTQ]?|[\d,]+)\s+Following/i,
  );
  const postsMatch = description.match(
    /(\d+(?:[.,]\d+)?[KMBTQ]?|[\d,]+)\s+Posts?/i,
  );

  if (followersMatch && followingMatch && postsMatch) {
    const usernameMatch =
      description.match(
        /(?:Posts\s*)?-\s*.*?\(@([\w.]+)\)\s*on\s*Instagram/i,
      ) || description.match(/@([\w.]+)/);
    const nameMatch = description.match(
      /Posts?\s*-\s*(.*?)(?:\s*\(@[\w.]+\)|@|on\s+Instagram)/i,
    );
    ...
    if (nameMatch && nameMatch[1].trim() && nameMatch[1].trim() !== "") {
      ...
      if (titleParts.length > 0) {
        ...
        if (
          titleName &&
          titleName !== extractedUsername &&
          !titleName.includes("Instagram")
        ) {
    ...
    const extractedBio = bioMatch
      ? bioMatch[1].trim().replace(/\s+/g, " ")
      : "";
...
router.get("/:username", async (req, res) => {
  ...
  try {
    ...
    if (!profile.description) {
      return res.status(404).json({
        error:
          "Nie udało się pobrać danych profilu - może być prywatny lub nieistniejący",
        username: username,
        pageTitle: profile.title,
      });
    }
    const parsedData = parseInstagramDescription(
      profile.description,
      username,
      profile.title,
    );
    ...
    if (
      error.message.includes("wykrył bota") ||
      error.message.includes("logowania")
    ) {
