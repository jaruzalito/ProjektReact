...
const loadFixtures = async () => {
  try {
    ...
    for (const profileData of profilesData) {
      ...
      console.log(
        `   ✓ Dodano profil: @${profileData.username} (${profileData.followers.toLocaleString()} followers)`,
      );
    }
    ...
    for (const commentData of commentsData) {
      ...
      const profile = profiles.find(
        (p) => p.username === commentData.profileUsername,
      );
      ...
      if (user && profile) {
        ...
        await comment.save();
        console.log(
          `   ✓ Dodano komentarz dla @${commentData.profileUsername} (ocena: ${commentData.rating}/10)`,
        );
      }
    }
    ...
    for (const ratingData of ratingsData) {
      ...
      const profile = profiles.find(
        (p) => p.username === ratingData.profileUsername,
      );
      ...
      if (user && profile) {
        ...
        await rating.save();
        console.log(
          `   ✓ Dodano ocenę dla @${ratingData.profileUsername} (${ratingData.value}/5)`,
        );
      }
    }
    ...
    for (const profile of profiles) {
      ...
      if (updatedProfile.avgRating) {
        console.log(
          `   ✓ @${profile.username}: średnia ${updatedProfile.avgRating.toFixed(1)}/10`,
        );
      } else {
